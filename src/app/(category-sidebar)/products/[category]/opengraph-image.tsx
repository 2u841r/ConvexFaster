...
export default async function Image(props: {
  ...
  const [category, subcollections] = await Promise.all([
    getCategoryBySlug(urlDecodedCategory),
    getSubcollectionsByCategory(urlDecodedCategory),
  ]);
  ...
  const examples = subcollections
    .slice(0, 2)
    .map((s) => s.name)
    .join(", ");

  const description = `Choose from our selection of ${category.name}, including ${examples + (subcollections.length > 1 ? "," : "")} and more. In stock and ready to ship.`;
